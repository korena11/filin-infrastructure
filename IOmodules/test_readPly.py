...
class TestReadPly(TestCase):
    def test_ReadPly_default(self):
        ...
        data = ReadFunctions.ReadPts(filename)
        ...
        self.assertEqual(len(data), 2, 'Unexpected number of returned from method')

        self.assertEqual(filename, data[0].path, 'path was not set')
        self.assertEqual(data[0].Size, 371973, 'Unexpected number of points in the load file')
