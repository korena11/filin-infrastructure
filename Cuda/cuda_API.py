...
def cumpute_normals_by_tensor_cuda(pntSet, neighborsNumber, neighbors):
    ...
    for i in range(1, partsNum + 1):
        ...
        if i * stride > numPnts:
            ...
            neighbors_device = cuda.to_device(neighbors[3 * ((i - 1) * stride):3 * numPnts, :])
            ...
            neighbors_device = cuda.to_device(neighbors[3 * ((i - 1) * stride):3 * (i * stride), :])
        ...
        if i * stride > numPnts:
            ...
            eigVectors_cpu[3 * numPnts - 3 * ((i - 1) * stride)] = eigVectors.copy_to_host()
        else:
            # tensors_cpu[3, 3 * numPnts - 3 * ((i - 1) * stride)] = tensors.copy_to_host()
