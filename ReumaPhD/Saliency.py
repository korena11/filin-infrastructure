...
def bayesian(image, **kwargs):
    ...
    img_x, img_y, img_xx, img_yy, img_xy = mt.computeImageDerivatives(image_gray, order = 2, sigma = inputs['sigma_dev'])
...
def __contextAware(image, ksize, image_feature, **kwargs):
    ...
    if image_feature == 'pixel_val':
        averaged_image = cv2.normalize(averaged_image, averaged_image, alpha = 0, beta = 1, norm_type = cv2.NORM_MINMAX, dtype = cv2.CV_32F)
